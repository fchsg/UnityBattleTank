...
public class TankIconSpineAttach : MonoBehaviour {
	...
	private int _bodyDegree;
	private int _cannonDegree;
	...
	private void UpdatePoser()
	{
		...
		float cannonDegree = _cannonDegree;
	...
	private void RefreshAnim()
	{
		...
		_animStartTimestamp = TimeHelper.GetCurrentRealTimestamp ();
	...
	public bool IsAnimFinished()
	{
		...
		long ts = TimeHelper.GetCurrentRealTimestamp ();
	...
	public void Attach(GameObject target, DataUnit dataUnit, string spineName, float scale, int bodyDegree = 90, int cannonDegree = 90)
	{
		...
		_target = target;
		_dataUnit = dataUnit;
		_bodyDegree = bodyDegree;
		_cannonDegree = cannonDegree;

		_spineName = spineName;
		_spineScale = _dataUnit.length * scale;
	...
	public Spine.Bone GetBoneMouse()
	{
		...
		if (_dataUnit.bodyType == DataConfig.BODY_TYPE.CAR) {
			boneName += _bodyStatus.displayLevel;
		} else if (_dataUnit.bodyType == DataConfig.BODY_TYPE.CAR_WITH_CANNON) {
